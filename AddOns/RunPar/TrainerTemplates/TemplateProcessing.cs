...
namespace Microsoft.Boogie
{
    public class RuleTemplate
    {
        ...
        public void LoadExprs()
        {
            ...
            program.TopLevelDeclarations.OfType<Procedure>().First().Ensures.Where(ens => QKeyValue.FindBoolAttribute(ens.Attributes, "pre")).ForEach(n => preSet.Add(n.Condition));
            program.TopLevelDeclarations.OfType<Procedure>().First().Ensures.Where(ens => QKeyValue.FindBoolAttribute(ens.Attributes, "post")).ForEach(n => postSet.Add(n.Condition));
        }

        private static string ListToDisjoin(HashSet<string> setStr, string prefix)
        {
            string buf = null;
            setStr.ForEach<string>(n => buf += prefix + " " + n + "; ");
            return buf;
        }
    ...
    public class EnvironmentalSummaries
    {
        ...
        public EnvironmentalSummaries(List<string> inputFiles, string dualityFixPointFile, bool addCorralOut)
        {
            ...
            fileList.ForEach<string>(n => Console.WriteLine(n));
    ...
    public class InductiveSummaries
    {
        ...
        public Dictionary<string, List<Expr>> trainSummaries(Program program, IEnumerable<RuleTemplate> templateDict, out HashSet<string> predicateSet)
        {
            ...
            CommandLineOptions.PrintErrorModel = 1;     // this is required
            string oldmodelfile = CommandLineOptions.PrintErrorModelFile;
            CommandLineOptions.PrintErrorModelFile = "model2.txt"; // else it is printed on STDOUT
            ...
            CommandLineOptions.PrintErrorModelFile = oldmodelfile;
